...
public class Dictionnaire extends Observable {
	...
	public int chercherIndiceVariableSortante(int jE)  {
		...
		boolean estBorne = false;
		int i0 = 0;
		double rapportMax = 0;
		for (int i=1; i<= nbVarBase; i++ ) {
			if ( (D[i][jE] < 0) && (-D[i][0]/D[i][jE] < rapportMax || !estBorne) ) {
				i0 = i;
				rapportMax = -D[i][0]/D[i][jE];
				estBorne = true;						
			}
	...
	public void pivoter(int iS, int jE) {
		...
		double facteur = -D[iS][jE];
		for(int j = 0 ; j<=nbVarHorsBase; j++) {
			D[iS][j] /= facteur;
		}
		double[] newCoeffs = new double[nbVarBase + 1];
		for (int i=0; i<= nbVarBase; i++) {
			if(i==iS) {
				newCoeffs[i] = -1/facteur;
			}
			else {
				newCoeffs[i] = D[i][jE] / D[iS][jE];
				for (int j=0; j<= nbVarHorsBase; j++) {
					D[i][j] -= newCoeffs[i]*D[iS][j];
		...
		int index = tabVarBase[iS];
		tabVarBase[iS] = tabVarHorsBase[jE];
		tabVarHorsBase[jE] = index;
	...
	public int chercherIndiceVariableEntranteAvantageuse() {	
		...
		int ind = 0;
		boolean estBorne;
		double augmentationMax = 0;
		double contrainteMin;
		...
		for (int j=1; j<= nbVarHorsBase; j++) {
			if(D[0][j] > 0) {
				...
				for (int i=0; i<= nbVarBase; i++) {
					...
					if (D[i][j] < 0 ) {
						estBorne = true;
						if ( -(D[i][0]/D[i][j])<contrainteMin ) {
							contrainteMin = -(D[i][0]/D[i][j]);
						}
				...
				if (-D[0][j]*contrainteMin < augmentationMax ) {
					augmentationMax = -D[0][j]*contrainteMin;
					ind = j;
				}
			}	
		}
