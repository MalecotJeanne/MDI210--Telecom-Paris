...
public class Descente extends Observable implements Runnable  {
	 ...
	 * @see Pb#gradientf(Couple)
	 * @see #seuil * 
	 * @param c la contrainte saturee par le point courant Pchaine direction dans l'attribut this.direction  
	 * (c'est obligatoire d'utiliser cet attribut)
	 */
	private void directionASuivreSiBord(Contrainte c) {
		Couple dir = pb.gradientf(P);
		Couple grad_c = c.getGradient();
		Couple bord_c = c.getVecteurUnitaireBord();
		if(dir.norme()<seuil) {
			this.finie = true;
		}
		else if(dir.produitScalaire(grad_c)>0) {
			this.direction = dir.mult(-1);
		}
		else {
			if (dir.estPerpendiculaire(bord_c)) {
				this.finie = true;
			}
			else if(dir.produitScalaire(bord_c)<0) {
				this.direction = bord_c;
			}
			else {
				this.direction = bord_c.mult(-1);
			}
		}
	}
	...
	private void directionASuivreSiCoin(Contrainte[] coin) {
		...
		if(dir.norme()<seuil) {
			...
			//on s'assure que les vecteurs unitaires sont bien orientés
			...
			Couple decompoGrad = Couple.decompose(dir, n1, n2);
			
			if(dir.produitScalaire(n1)<0 && dir.produitScalaire(n2)<0) {
			//zone 1
				this.direction = dir.mult(-1);				
			}
			else if(decompoGrad.x>0 && decompoGrad.y>0) {
			//zone 2
				this.finie = true;
			}
			else if(dir.produitScalaire(u1)<dir.produitScalaire(u2)){
			//zone 3	
				this.direction = u1;
			}
			else {
	...
	public double chercheSecondPoint(Couple pointDepart, Couple dir) {
		double t = 1;
		double phid = pb.phiDerivee(pointDepart, dir, t);
		while(phid<0 && t<Double.MAX_VALUE/2) {
			t *=2;
			phid = pb.phiDerivee(pointDepart, dir, t);
		}
	 ...
	 * 					<br>- le couple forme par le multiplicateur de Lagrange et 0 si P est sur un bord
	...
	public Couple KarushKuhnTucker(Couple P) {
		...
		if(domaine.estCoin(P)!=null) {
			...
			if (decompoGrad.x < 0 && decompoGrad.y < 0) {
				lambda = decompoGrad;
			}
		}
			...
			Couple nBord = bord.getGradient().mult(-1);
			Couple uBord = bord.getVecteurUnitaireBord();
			
			if(!dir.estPerpendiculaire(uBord)) {
				lambda = new Couple(dir.norme()/nBord.norme(), 0);
			}
		}
		
		
		else if(dir.norme()<seuil) {
